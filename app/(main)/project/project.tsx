...
function GlassDesc({ children, className = "" }: GlassDescProps) {
  return (
...
function ProjectRow({ side = "left", title, tag, image, desc }: ProjectRowProps) {
  ...
  return (
    ...
    >
      ...
      <div className={`${left ? "order-1" : "order-2"} flex flex-col`}>
        ...
        <h3 className="mt-1 text-[28px] md:text-[30px] font-semibold text-white">{title}</h3>
        ...
        <div className="relative rounded-[16px] overflow-hidden shadow-[0_25px_80px_rgba(44,18,84,0.55)]">
          ...
          <div className="relative bg-[#1B1234] p-4 md:p-5">
            <div className="rounded-[12px] overflow-hidden bg-white shadow-[inset_0_0_0_1px_rgba(0,0,0,0.06)]">
              <Image src={image} alt={`${title} screenshot`} width={800} height={500} className="block w-full h-auto" />
            </div>
          </div>
          <div className={`absolute ${left ? "right-10" : "left-10"} -bottom-2 h-4 w-28 rounded-md bg-black/35 blur-md`} />
        </div>
        <div className={`absolute -z-10 ${left ? "right-6" : "left-6"} bottom-5 h-7 w-40 rounded-full bg-black/30 blur-2xl`} />
      </div>
    </div>
  );
}
